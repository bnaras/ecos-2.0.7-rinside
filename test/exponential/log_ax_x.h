...
static char * test_log_ax_x(RInside *R){
pwork *mywork;
idxint exitflag;
...
R->assign(logax_x_n, "n");
R->assign(logax_x_m, "m");
R->assign(logax_x_p, "p");
R->assign(logax_x_l, "l");
R->assign(logax_x_ncones, "ncones");
R->assign(iVec(NULL, ILEN(NULL)), "q");
R->assign(logax_x_nexc, "e");
R->assign(dVec(logax_x_Gpr, DLEN(logax_x_Gpr)), "Gpr");
R->assign(iVec(logax_x_Gjc, ILEN(logax_x_Gjc)), "Gjc");
R->assign(iVec(logax_x_Gir, ILEN(logax_x_Gir)), "Gir");
R->assign(dVec(NULL, DLEN(NULL)), "Apr");
R->assign(iVec(NULL, ILEN(NULL)), "Ajc");
R->assign(iVec(NULL, ILEN(NULL)), "Air");
R->assign(dVec(logax_x_c, DLEN(logax_x_c)), "c");
R->assign(dVec(logax_x_h, DLEN(logax_x_h)), "h");
R->assign(dVec(NULL, DLEN(NULL)), "b");
std::string fname = "log_ax_x_1.RDS";
R->assign(fname, "fname");
R->parseEvalQ("log_ax_x <- list(n=n, m=m, p=p, l=l, ncones=ncones, q=q, e=e, Gpr=Gpr, Gjc=Gjc, Gir=Gir, Apr=Apr, Ajc=Ajc, Air=Air, c=c, h=h, b=b)");
R->parseEvalQ("saveRDS(log_ax_x, file=fname)");
